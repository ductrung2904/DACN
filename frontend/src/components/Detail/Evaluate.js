...
function Evaluate({ evaluates }) {
    ...
    const displayUsers = evaluates.slice(pagesVisited, pagesVisited + usersPerPage).map((evaluate) => {
        return (
            <>
                <div className="evaluate__item">
                    <div className="evaluate__avatar d-flex align-items-center">
                        ...
                        <div className="evaluate__info">
                            <div class="evaluate__info__content">
                                <span className="evaluate__info__name">{evaluate.cus_name}</span>
                                <span className="evaluate__info__bought">
                                    <span className="ico-tick"></span>
                                    Đã mua hàng ở Tiki
                                </span>
                            </div>
                            <div className="evaluate__info__content">
                                <span className="evaluate__date">Nhận xét vào {new Date(evaluate.eva_date).toLocaleDateString('vi-VN', { year: 'numeric', month: '2-digit', day: '2-digit' })}</span>
                                <span className="evaluate__seller"> Mua từ nhà bán {evaluate.shop_name}</span>
                            </div>
                        </div>
                    </div>
                    <div className="evaluate__rate">
                    ...
                    <div className="evaluate__images">
                        {moreImg(evaluate)}
                    </div>
                    <span className="evaluate__thank"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                        viewBox="0 0 20 20">
                    ...
                    </svg><span>Hữu ích</span></span><span className="evaluate__reply">Gửi trả lời</span>
                </div>
            </>
        )
    });

    const pageCount = Math.ceil(evaluates.length / usersPerPage);

    const changePage = ({ selected }) => {
        setPageNumber(selected);
        // window.scrollTo({ behavior: "smooth", top: 2000 });
        document.querySelector(".evaluate__item").scrollIntoView({ behavior: 'smooth' })
    };
